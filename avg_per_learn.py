        ...
        for each_file in total_training_files:
            ...
            if((file_label * result > 0) != 1):
                for word in number_of_words.keys():
                    ...
                    temp_dict[word] = file_label*facto*number_of_words[word] + temp_dict[word]
                temp_res = temp_res + file_label
                temp_fact = file_label*facto + temp_fact
            facto = facto + 1

    for word in temp_dict:
        temp_var2 = 1/facto
        temp_dict[word] = weights[word] - temp_var2*temp_dict[word]

    temp_fact = temp_res - (1/facto)*temp_fact
    return (temp_dict,temp_fact)
